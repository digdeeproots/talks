...
1. **Hydrate working memory** from the session file — task list, map of named text sequences ("documents"), typed-block DAG (block is text sequence). *(Memory lever.)*
2. **Set the turn goal** from input. *(Goals lever / Goal axis.)*
