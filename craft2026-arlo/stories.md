...
2. **Extract sequencing into deterministic code.** Claude can't clear its own context or exit itself. Claude wrote a script — `do-today` — that runs one step, does deterministic processing, launches Claude, catches the result, and quits. Run again for the next step. Sequencing is no longer in Claude's hands.
